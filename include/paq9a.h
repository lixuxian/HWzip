...
mixing compressor and arithmetic coded one bit at a time.  Mode_paql
...
typedef enum {COMPRESS, DECOMPRESS} Mode_paq;
class Encoder {
private:
  const Mode_paq mode;       // Compress or decompress?
...
Encoder::Encoder(Mode_paq m, FILE* f):
...
void paq9a(FILE* in, FILE* out, Mode_paq mode) {
  if (!lzp && !predictor) {
    lzp=new LZP;
